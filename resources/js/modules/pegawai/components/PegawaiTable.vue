        ...
        <div class="card-body">
                    ...
                    <tbody>
                        <tr v-if="isLoading">
                            <td colspan="6" class="text-center p-5">
                                <div class="spinner-border text-primary" pegawai="status"></div>
                                <p class="mt-2 mb-0">Memuat data...</p>
                            </td>
                        </tr>

                        <tr v-else-if="!paginatedPegawai || paginatedPegawai.length === 0">
                                ...
                                <td class="text-start">
                                    <h2 class="table-avatar d-inline-flex align-items-center mb-0">
                                        <span class="avatar avatar-sm me-2">
                                            <img class="avatar-img rounded-circle" :src="getImageUrl(item.image)"
                                                alt="User Image"
                                                @error="(e) => e.target.src = '/assets/img/profiles/avatar-01.jpg'">
                                        </span>
                                        <span style="font-size: 14px;" class="text-dark"><b>{{ item.nama }}</b></span>
                                    </h2>
                                </td>
                                <td>{{ item.kontak }}</td>
                                <td>{{ item.alamat }}</td>
            ...
            <div v-if="filteredPegawai.length > 0" class="d-flex justify-content-between align-items-center p-3">
                ...
                <div class="text-muted small">
                    ...
                    to {{ Math.min(currentPage * 10, filteredPegawai.length) }}
                    of {{ filteredPegawai.length }} entries
                </div>
