<template>
    <div class="card card-table">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-nowrap table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <template v-else>
                            <tr v-for="(item, index) in paginatedSuplier" :key="item.id" class="text-center">
                                ...
                                <td>{{ item.kontak }}</td>
                                <td>{{ item.alamat }}</td>
                                <td>{{item.masterplants?.map(plant => plant.kode).join(', ') || 'Tidak ada plant'}}
                                </td>
                                <td>
            ...
            <div v-if="filteredSuplier.length > 0" class="d-flex justify-content-between align-items-center p-3">

                <div class="text-muted small">
                    Showing {{ ((currentPage - 1) * 10) + 1 }}
                    to {{ Math.min(currentPage * 10, filteredSuplier.length) }}
                    of {{ filteredSuplier.length }} entries
                </div>
