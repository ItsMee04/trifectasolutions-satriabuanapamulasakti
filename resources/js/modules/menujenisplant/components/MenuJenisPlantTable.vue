        ...
        <div class="card-body">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h5 class="card-title">Daftar Master Plant</h5>
                    </div>
            ...
            <div class="table-responsive">
                <table class="table table-nowrap table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="!paginatedMenuJenisPlant || paginatedMenuJenisPlant.length === 0">
                            <td colspan="6" class="text-center p-5">Tidak ada data.</td>
                        </tr>

                        <template v-else>
                            <tr v-for="(item, index) in paginatedMenuJenisPlant" :key="item.id" class="text-center">
                                <td>{{ ((currentPage - 1) * 10) + (index + 1) }}</td>
                                <td>{{ item.masterplant?.plant }}</td>
                                <td>{{ item.menujenis }}</td>
                                <td>
            ...
            <div v-if="filteredMenuJenisPlant.length > 0" class="d-flex justify-content-between align-items-center p-3">

                <div class="text-muted small">
                    Showing {{ ((currentPage - 1) * 10) + 1 }}
                    to {{ Math.min(currentPage * 10, filteredMenuJenisPlant.length) }}
                    of {{ filteredMenuJenisPlant.length }} entries
                </div>
...
const {
    ...
    displayedPages,
    filteredMenuJenisPlant,
    paginatedMenuJenisPlant,
    searchQuery,
    isLoading,
    currentPage,
    totalPages,
