<template>
    <div class="card card-table">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-nowrap table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="!paginatedKategori || paginatedKategori.length === 0">
                            <td colspan="4" class="text-center p-5">Tidak ada data.</td>
                        </tr>

                        <template v-else>
                            <tr v-for="(item, index) in paginatedKategori" :key="item.id" class="text-center">
            ...
            <div v-if="filteredKategori.length > 0" class="d-flex justify-content-between align-items-center p-3">

                <div class="text-muted small">
                    Showing {{ ((currentPage - 1) * 10) + 1 }}
                    to {{ Math.min(currentPage * 10, filteredKategori.length) }}
                    of {{ filteredKategori.length }} entries
                </div>
